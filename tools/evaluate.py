...
def cat_pc(gt, predictions, thresholds):
    ...
    for i, p in enumerate(predictions):
        ...
        try:
            ...
            gt_boxes = image_gt_boxes[p['name'] + '.jpg']
            gt_checked = image_gt_checked[p['name'] + '.jpg']
        except KeyError:
            gt_boxes = []
            gt_checked = None
...
def evaluate_detection(gt_path, result_path):
    ...
    print('{:.2f}'.format(mean), ', '.join(['{:.2f}'.format(n) for n in breakdown]))
...
def main():
    # evaluate_detection('../data/bdd100k/labels/gt_val.json', '../data/val_result.json')
